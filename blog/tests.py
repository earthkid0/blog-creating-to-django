...
class TestView(TestCase):
    ...
    def test_post_list(self):
        ...
        self.assertIn(self.post_003.author.username.lower(), post_003_card.text)
    ...
    def test_category_page(self):
        ...
        self.assertIn(self.category_고추.name, soup.h1.text)

        main_area = soup.find('div', id='main-area')
        self.assertIn(self.category_고추.name, main_area.text)
        self.assertIn(self.post_001.title, main_area.text)
        self.assertNotIn(self.post_002.title, main_area.text)
        self.assertNotIn(self.post_003.title, main_area.text)

    def test_create_post(self):
        #로그인을 하지 않으면 status code가 200이 안된다.
    ...
    def test_comment_form(self):
        ...
        self.assertIn('earthkid0', new_comment_div.text)
        self.assertIn('earthkid0의 댓글입니다.', new_comment_div.text)
    
    def test_comment_update(self):
        comment_by_deadbort = Comment.objects.create(
            post = self.post_001,
            author = self.user_deadbort,
            content = 'deadbort의 댓글입니다.'
        )
        response = self.client.get(self.post_001.get_absolute_url())
        self.assertEqual(response.status_code, 200)
        soup = BeautifulSoup(response.content, 'html.parser')
        ...
        comment_area = soup.find('div', id='comment-area')
        self.assertFalse(comment_area.find('a', id='comment-2update-btn'))
        ...
        self.assertIn('earthkid0의 댓글을 수정합니다.', comment_001_div.text)
    ...
    def test_delete_comment(self):
        ...
        self.assertIn(self.post_001.title, soup.title.text)
        comment_area = soup.find('div', id='comment-area')
        self.assertNotIn('deadbort의 댓글입니다.', comment_area.text)
